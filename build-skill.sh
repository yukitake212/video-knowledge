...
# --- 4. マーカーの残留検査 ------------------------------------------------
# inject_marker の接続漏れは終了コード0で完了してしまい、出力を見ないと気づけない。
# 実際に GENRES を足したとき、パイプラインへの接続を落としたまま「ビルド完了」が出た。
#
# 対象は生成した SKILL.md 系だけ。reference/ は正本のコピーで、将来ふつうの
# HTML コメントが入る可能性があり、そこで落ちると原因が分からなくなる。
# パターンは大文字のマーカーに限る（通常のコメントを誤検出しないため）。
for f in "$DIST/skill/SKILL.md" "$DIST/skill-local/SKILL.md"          "$DIST/chatgpt/INSTRUCTIONS.md" "$DIST/chatgpt/video-knowledge-bundle.md"; do
  [ -f "$f" ] || die "生成されるはずの $f が無い"
  # grep はマッチ0件で 1 を返す。pipefail 下ではそれが正常系なので吸収する。
  left="$( { grep -oE '<!-- [A-Z_]+ -->' "$f" || true; } | sort -u | tr '
' ' ')"
  [ -z "$left" ] || die "$f にマーカーが残っている: ${left}（inject_marker の接続漏れ）"
done
