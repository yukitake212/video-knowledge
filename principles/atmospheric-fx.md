...
**公開Skillは「薄い霧」ではなく `fog density 40%`、`haze visible at 15 meters depth` と書けとしている。**

**絶対値としては採らない**（2026-09-02 / 指摘を受けて格下げ）。
**モデルの内部に標準化された霧密度の尺度がある根拠が無い。** 40% は擬似精密。

**使えるのは相対の側。ショットをまたいで段階的に上げる**——20% → 40% → 60%。
**数値そのものではなく、増えていく順序が効く**（これも未検証）。
