...
## 通ったプロンプトの要素（自社検証・2026-08-31、2件成功）

- **参照の切り分けを冒頭に置く**——「顔と髪型だけ取る／背景・ボケ・照明・フレーミングは取らない」
- **顔についての記述を足さない。年齢も体格も顔つきも書かない**
- 衣装は一点ずつ数え上げ、**外す小物まで書く**（バッグ・時計・ネックレス・ランヤードなし）
- 顔は1枚だけ、全身はヘッドレス
- 顔アップに襟元をほぼ入れない
- 文字を一切入れない
- 参照に複数人が写っている場合、**対象を肯定で名指ししてから、他方を否定で除外する**

**このうち「参照が持つ情報を書き直さない」が最も効いた。**
書き直すと**モデルが参照より文章を優先し、髪型と肉付きが動く。**

## 確定済みのキャラを型に合わせて作り直すのはリスク（自社検証・2026-08-31）

確定済みキャラを**型（顔1枚＋ヘッドレス全身2体）で作り直した結果、
顔が3回続けて別人化した。** 最終的に**3面図（顔が3つ並ぶ）で解決した。**

型が顔1枚を推奨しているのは「シート内に顔が複数あると動画モデルがドリフトする」
という理由だが、**3面図が実務で成立しないわけではない。**
別キャラの3面図は、そこからの生成で問題なく機能した。

**型に合わせようとして同一性を壊すのは本末転倒。**
**型で作るのは新規キャラを最初から作るときで、確定済みのキャラをシート化し直す必要はない。**

**なお Seedream 5 Pro に3面図を参照として渡すと、3面図の版面ごと返ってくる**
（→ `models/image/seedream.md`）。
