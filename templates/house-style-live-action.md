...
- **この「前置きで1回貼る」構造そのものに、反対の公開情報がある。**
  Seedance 向けの公開Skillは「**先頭にスタイルブロックを置くな。
  光は LIGHTING、色は COLOR GRADE か LOCATION、演技と肌は PERFORMANCE、
  というふうに、それを支配するブロックの中へ分散させろ**」としている。
  **技術系（解像度・グレイン・fps）だけを末尾のサフィックスに置く**、という配置。
  **どちらの根拠も公開プロンプトの観察で、n が小さい。**
  この骨格を「界隈の共通様式」としたのも公開プロンプトの観察なので、**確定していない**
  （2026-09-02 / `corpus/research/higgsfield-seedance-skill-2026-09/`。**自社未検証**）
- **Light節に露出方針を1行入れられる。**「暗い」を繰り返すのではなく、
  **意図的にアンダー／シャドウを守る／ハイライトは抑えるがテクスチャを残す**と**方針**で書く。
  公開クリップはこの3点を独立した節として書いていた（2026-09-02 / 外部・**自社未検証**）
